...
import { eliminarFavorito } from "../services/auth.services";
...
function Favoritos() {
  ...
  async function handleEliminarFavorito(item) {
    const result = await eliminarFavorito(item.favorito_id);
      ...
      {state.Favoritos.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {state.Favoritos.map((item) => (
            <div
              key={item.favorito_id}
              className="group relative shadow-lg transform transition-all duration-300 hover:z-20 hover:-translate-y-4 hover:scale-105 hover:shadow-2xl hover:ring-red-500/50"
            >
              <div className="relative h-[400px] rounded-2xl overflow-hidden">
                ...
                <div className="absolute top-0 left-0 right-0 flex items-end justify-between gap-3 p-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-500">
                  ...
                  <i className="fa-solid fa-heart cursor-pointer transition transform pt-2 hover:scale-150 text-red-500 scale-110"></i>
                </div>
                ...
                <i
                  ...
                  onClick={() => handleEliminarFavorito(item)}
                ></i>
              </div>
